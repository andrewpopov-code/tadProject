...
def _sa_block(
    ...
    wrapper.Filtration(d, wrapper.global_step)
    wrapper.ID(d, wrapper.global_step)
    wrapper.log(d)
...
class TransformerEncoderWrapper(nn.Module):
    def __init__(self, encoder: nn.TransformerEncoder, writer: SummaryWriter,
                 homology_dim=1, display_heads=False, display_heatmap: int = 1):
        ...
        self.Filtration = Persistence(writer, encoder.num_layers, homology_dim, display_heatmap)
        self.ID = IntrinsicDimension(writer, encoder.num_layers)
    ...
    def log(self, d: torch.Tensor):
        if self.encoder_step % self.display_heatmap == 0:
            draw_heatmap(
